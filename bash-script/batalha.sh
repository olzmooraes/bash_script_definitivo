...
if [[ ${speed_pkm1[2]} -gt ${speed_pkm2[2]} ]]; then

    while [[ $vida_pkm1 -gt 0 && $vida_pkm2 -gt 0 ]]; do

        if [[ $vida_pkm2 -gt 0 ]]; then

            ataque_final_pkm1= $((ataque_pkm1 - $defesa_pkm2))
            vida_pkm2= $((vida_pkm2 - $ataque_final_pkm1))
            j=$((j + 1))
        fi
        if [[ $vida_pkm1 -gt 0 ]]; then
...
#if [[ ${speed_pkm2[2]} > ${speed_pkm2[2]} ]]; then
